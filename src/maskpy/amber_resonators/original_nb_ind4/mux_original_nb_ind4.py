...
def get_freq_bands() -> tuple[
    ...
    low_band_min_max_freqs = (868755650.3808191, 885558722.917953)
    mid_band_min_max_freqs = (966482316.074448, 990372138.100482)
    high_band_min_max_freqs = (1104384600.793312, 1141602974.241003)
...
def get_IDCLs_and_CCL_from_f0(
    ...
    if low_band_min_max_freqs[0] <= f0 <= low_band_min_max_freqs[1]:
        fit = np.poly1d([-1.09748163e-19, 2.85624232e-10, -2.47977033e-01, 7.18242446e07])
        no_of_arms = 21
    elif mid_band_min_max_freqs[0] <= f0 <= mid_band_min_max_freqs[1]:
        fit = np.poly1d([-3.73816166e-20, 1.08171839e-10, -1.04476145e-01, 3.36836385e07])
        no_of_arms = 17
    elif high_band_min_max_freqs[0] <= f0 <= high_band_min_max_freqs[1]:
        fit = np.poly1d(np.array([-9.39137488e-21, 3.10093531e-11, -3.42174539e-02, 1.26221417e07]))
        no_of_arms = 13
    else:
        raise ValueError(
