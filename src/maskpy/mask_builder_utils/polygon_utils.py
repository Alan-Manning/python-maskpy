...
def mirror_points_around_yaxis(points: Sequence[Sequence[float]]) -> list[list[float]]:
...
def mirror_points_around_xaxis(points: Sequence[Sequence[float]]) -> list[list[float]]:
...
def get_intersection_point_from_two_points_and_angles(
    p1_x: float,
    p1_y: float,
    angle_1: float,
    p2_x: float,
    ...
    """Get the intersection point between the lines extending from point 1 and
    point 2 at given angles angle_1 and angle_2 respectively. Each angle should
    be in radians where 0 angle is a line pointing in the positive x direction.
    ...
    angle_1: float,
        The angle (**in radians**) for the line extending from point 1.
    ...
    angle_2: float,
        The angle (**in radians**) for the line extending from point 2.

    Returns
    -------
    mid_point: list[float]
        The [x, y] coordinate for the intersection.
    """
    ...
    # KEEP
    # if not np.any(np.isclose([angle_1, angle_1], [0, pi])):
    #     u_numerator = p1_x - (p1_y / tan(angle_1)) - (p2_y / tan(angle_1)) - p2_x
    #     u_demoninator = cos(angle_2) - (sin(angle_2) / tan(angle_1))
    # else:
    #     u_numerator = p1_x - (p1_y * cos(angle_1) / sin(angle_1)) - (p2_y * cos(angle_1) / sin(angle_1)) - p2_x
    #     u_demoninator = cos(angle_2) - (sin(angle_2) * cos(angle_1) / sin(angle_1))
    # KEEP

    u_numerator = (p1_x * sin(angle_1)) - (p1_y * cos(angle_1)) + (p2_y * cos(angle_1)) - (p2_x * sin(angle_1))
    u_demoninator = (cos(angle_2) * sin(angle_1)) - (sin(angle_2) * cos(angle_1))
    if np.isclose(u_demoninator, 0):
        u_numerator = (p1_y * cos(angle_1)) - (p1_x * sin(angle_1)) + (p2_x * sin(angle_1)) - (p2_y * cos(angle_1))
        u_demoninator = (sin(angle_2) * cos(angle_1)) - (cos(angle_2) * sin(angle_1))
    if np.isclose(u_demoninator, 0):
        raise ZeroDivisionError("here.")

    u = u_numerator / u_demoninator

    x = p2_x + u * cos(angle_2)
    y = p2_y + u * sin(angle_2)
...
def get_intersection_point_and_curve_radius(
    p1_x: float,
    p1_y: float,
    angle_1: float,
    p2_x: float,
    ...
    """Get the intersection point between the lines extending from point 1 and
    point 2 at given angles angle_1 and angle_2 respectively and also the bend
    radius for a circular curve that smoothly connects the two points. Each
    angle should be in radians where 0 angle is a line pointing in the positive
    x direction. It may not be possible to smoothly connect the two points in a
    circular path so in that case the bend radius will be as large as possible
    to facilitate a smooth arc extending into a straight section to finish the
    connection.
    ...
    Returns
    -------
    (mid_point, bend_radius): tuple[list[float], float]
        The [x, y] coordinate for the intersection and the bend radius for
        connecting those points in an circular curve.
    """
    middle_xy = get_intersection_point_from_two_points_and_angles(
        p1_x,
        p1_y,
        angle_1,
        p2_x,
    ...
    if np.isclose(angle_1, angle_2):
        min_r = 0.0
        pretty_print(f"Warning: angle_1 and angle_2 are too similar to create circular arc. radius of 0.0 returned.")
        return (middle_xy, min_r)
    ...
    r_from_x = (p1_x - p2_x) / (cos(phi) - cos(theta))
    r_from_y = (p1_y - p2_y) / (sin(phi) - sin(theta))

    min_r = min(abs(r_from_x), abs(r_from_y))
