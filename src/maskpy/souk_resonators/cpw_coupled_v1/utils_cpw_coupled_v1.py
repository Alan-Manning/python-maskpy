...
def _this_resonator_type() -> SoukResonatorType:
    return SoukResonatorType.CPW_COUPLED_V1


def get_total_height_of_resonator(resonator_config_override: dict[str, float | int] | None = None) -> float:
    ...
    total_resonator_height = (
        config["ant_microstrip_final_y_offset"]
        ...
        + (5 * config["step_down_distance_between_layers"])
    )

    return total_resonator_height


def get_horizontal_coupler_end_to_meander_base_distance(resonator_config_override: dict[str, float | int] | None = None) -> float:
...
def get_vertical_coupler_center_to_meander_base_distance(resonator_config_override: dict[str, float | int] | None = None) -> float:
    ...
    coupler_center_to_meander_base_distance = (
        config["ant_microstrip_final_y_offset"]
        + config["meander_cpw_outer_width"] / 2
        + config["meander_left_height_1"]
        + config["meander_left_height_2"]
        + config["meander_left_height_3"]
        + config["meander_left_height_4"]
...
def get_width_and_height_of_IDC_cutout_section(resonator_config_override: dict[str, float | int] | None = None) -> tuple[
    float | int,
    float | int,
]:
