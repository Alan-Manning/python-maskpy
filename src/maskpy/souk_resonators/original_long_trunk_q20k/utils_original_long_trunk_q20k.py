from .config_original_long_trunk_q20k import get_resonator_config
...
def get_width_and_height_of_IDC_cutout_section(config_override: dict[str, float | int] | None = None) -> tuple[
    ...
    left_side_width = (
        (config["meander_bot_width"] / 2)
        - (config["meander_lw"] / 2)
        + (config["meander_left_width_1"] - config["meander_left_width_2"])
        + config["frame_bot_left_width"]
    ...
    right_side_width = (
        (config["meander_bot_width"] / 2)
        - (config["meander_lw"] / 2)
        + (config["meander_right_width_1"] - config["meander_right_width_2"])
